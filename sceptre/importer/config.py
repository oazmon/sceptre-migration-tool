...
def import_config(
    ...
    with open(config_path, 'w') as config_file:
        print('template_path: ' + template.relative_template_path, file=config_file)
        print('stack_name: ' + aws_stack_name, file=config_file)
        ...
        if 'RoleARN' in response['Stacks'][0] \
            ...
            print('role_arn: '+ response['Stacks'][0]['RoleARN'], file=config_file)
        ...
        if 'Parameters' in response['Stacks'][0]:
            ...
            parameter_list = response['Stacks'][0]['Parameters'] \
                if response['Stacks'][0]['Parameters'] \
                else []
            print('parameters:', file=config_file)
            for parameter in sorted(parameter_list, key=lambda x: x['ParameterKey']):
        ...
        if 'Tags' in response['Stacks'][0] \
            ...
            for tag in response['Stacks'][0]['Tags']:
                print('  {}: {}'.format(tag['Key'], tag['Value']), file=config_file)
